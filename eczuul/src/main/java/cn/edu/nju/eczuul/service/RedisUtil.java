...
import cn.edu.nju.eczuul.redis.KeyPrefix;
...
public class RedisUtil {
    ...
    public void del(KeyPrefix prefix, String... key) {
        if (key!=null && key.length>0) {
            if (key.length == 1) {
                redisTemplate.delete(prefix.getPrefix() + key[0]);
            } else {
                String[] keys = new String[key.length];
                for (int i = 0;i < key.length;i++){
                    keys[i] = prefix.getPrefix() + key[i];
                }
    ...
    public long sSetAndTime(String key, long time, Object... values) {
        try {
            Long succeedCount = redisTemplate.opsForSet().add(key, values);
            if (time > 0) {
                expire(key, time);
            }
            return succeedCount;
